...
==== Humio
Humio registry pushes metrics to https://cloud.humio.com[humio] periodically. To
export metrics to {micrometer-registry-documentation}/humio[Humio], your API token must
be provided:
...
You can also change the interval at which metrics are sent to Humio:

[source,properties,indent=0]
----
	management.metrics.export.humio.step=30s
----
